...
def plot_results(results: Dict[str, Any], save_path: str = None):
    ...
    try:
        ...
        weights_df[['weights']].apply(lambda x: pd.Series(x['weights']), axis=1).plot(ax=ax3)
        ax3.set_title('策略权重变化')
        ax3.set_xlabel('日期')
        ax3.set_ylabel('权重')
        
        # 绘制市场环境
        market_regime = weights_df['market_regime'].value_counts()
        market_regime.plot(kind='bar', ax=ax4)
        ax4.set_title('市场环境分布')
        ax4.set_xlabel('市场环境')
        ax4.set_ylabel('天数')
...
def main():
    ...
    try:
        # 设置回测参数
        start_date = '2024-01-01'  # 使用2023年的数据
        end_date = '2025-04-13'
        symbol = 'GOOG'  # 使用谷歌股票
        ...
        logger.info("\n=== 策略权重统计 ===")
        weights_df = pd.DataFrame(results['weights_history'])
        avg_weights = weights_df['weights'].apply(lambda x: pd.Series(x)).mean()
        ...
        logger.info("\n=== 市场环境统计 ===")
        market_regime = weights_df['market_regime'].value_counts()
        for regime, count in market_regime.items():
            logger.info(f"{regime}: {count}天 ({count/len(weights_df)*100:.2f}%)")
