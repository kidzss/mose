...
class CPGWStrategy(Strategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Generate signals based on RSI and MA crossover
        for i in range(1, len(df)):
            # Buy signal: RSI < oversold and MA crossover is positive
            if df['rsi'].iloc[i-1] < self.oversold and df['ma_cross'].iloc[i] == 1:
                df.loc[df.index[i], 'signal'] = 1
            
            # Sell signal: RSI > overbought and MA crossover is negative
            elif df['rsi'].iloc[i-1] > self.overbought and df['ma_cross'].iloc[i] == -1:
                df.loc[df.index[i], 'signal'] = -1
    ...
    def get_market_regime(self, df: pd.DataFrame) -> MarketRegime:
        ...
        # Get recent trend direction using longer MA
        ...
        elif ma_50 > ma_200 * 1.05:  # Strong uptrend
            return MarketRegime.BULLISH
        elif ma_50 < ma_200 * 0.95:  # Strong downtrend
