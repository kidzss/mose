...
class DataFetcher:
    ...
    def get_historical_data(
        ...
        """
        获取历史数据，优先从数据库获取，如果数据库没有则从API获取
        ...
        try:
            ...
            if self.cache_data and cache_key in self.data_cache:
                cache_time = self.cache_timestamps.get(cache_key, 0)
                if time.time() - cache_time < self.cache_expiry:
                    self.logger.debug(f"从缓存获取 {symbol} 的历史数据")
                    return self.data_cache[cache_key]
            
            # 尝试从数据库获取
            if use_db:
                db_data = self._get_data_from_db(symbol, start_date, end_date)
            ...
            # 如果数据库没有数据，从API获取
            api_data = self._get_data_from_api(symbol, start_date, end_date)
            
            # 更新缓存
            if self.cache_data and not api_data.empty:
                self.data_cache[cache_key] = api_data
                self.cache_timestamps[cache_key] = time.time()
                
            return api_data
        except Exception as e:
            self.logger.error(f"获取 {symbol} 的历史数据时出错: {e}")
            return pd.DataFrame()
    ...
    def get_realtime_data(self, symbols: List[str]) -> pd.DataFrame:
        ...
        try:
            ...
            if self.cache_data and cache_key in self.data_cache:
                cache_time = self.cache_timestamps.get(cache_key, 0)
                # 实时数据缓存时间更短
                if time.time() - cache_time < 60:  # 1分钟
                    self.logger.debug(f"从缓存获取实时数据")
                    return self.data_cache[cache_key]
            
            self.logger.info(f"获取 {len(symbols)} 只股票的实时数据...")
            
            # 使用线程池并发获取数据
            all_data = []
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                future_to_symbol = {
                    executor.submit(self._get_single_realtime_data, symbol): symbol
                ...
                for future in as_completed(future_to_symbol):
                    ...
                    try:
                        ...
                        self.logger.error(f"获取 {symbol} 的实时数据时出错: {e}")
            
            if not all_data:
                self.logger.warning("未获取到任何实时数据")
                return pd.DataFrame()
            ...
            self.logger.error(f"获取实时数据时出错: {e}")
            return pd.DataFrame()
            
    def _get_single_realtime_data(self, symbol: str) -> Optional[pd.DataFrame]:
        """
        获取单个股票的实时数据
        ...
        try:
            ...
            # 计算变化
            if last_close:
                data['Change'] = data['Close'] - last_close
                data['ChangePercent'] = (data['Change'] / last_close) * 100
            else:
                data['Change'] = 0
            ...
            self.logger.error(f"获取 {symbol} 的实时数据失败: {e}")
            return None
            
    def _get_last_close(self, symbol: str) -> Optional[float]:
        """获取上一个交易日的收盘价"""
        try:
            # 获取最近的历史数据
            end_date = dt.datetime.now().strftime('%Y-%m-%d')
            ...
            data = self._get_data_from_db(symbol, start_date, end_date)
            ...
            if not data.empty:
                # 获取最后一个交易日的收盘价
                return data['Close'].iloc[-2] if len(data) > 1 else None
                
            return None
        except Exception as e:
            self.logger.error(f"获取 {symbol} 的上一个收盘价失败: {e}")
            return None
