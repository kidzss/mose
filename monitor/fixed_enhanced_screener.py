...
logger = logging.getLogger("FixedEnhancedScreener")
...
class FixedEnhancedScreener:
    ...
    def __init__(self, min_market_cap: float = 5e8, min_avg_volume: float = 1e5):
        ...
        logger.info("🚀 修复版增强专业多策略量化筛选器初始化完成")
    ...
    def standardize_data_columns(self, data: pd.DataFrame) -> pd.DataFrame:
        """标准化数据列名，确保策略兼容性"""
        try:
            if data.empty:
                return data
    ...
    def calculate_technical_score(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            ...
            macd_data = self.technical_indicators.calculate_macd(close)
            bb_data = self.technical_indicators.calculate_bollinger_bands(close)
            adx = self.technical_indicators.calculate_adx(high, low, close)
            
            scores = {}
            
            # 1. 趋势指标评分
            trend_score = self._calculate_trend_score(data, bb_data, adx)
            scores['trend'] = trend_score
            
            # 2. 动量指标评分
            momentum_score = self._calculate_momentum_score(data, macd_data)
            scores['momentum'] = momentum_score
            
            # 3. 超买超卖指标评分
            overbought_oversold_score = self._calculate_overbought_oversold_score(rsi)
    ...
    def _calculate_trend_score(self, data: pd.DataFrame, bb_data: Dict, adx: pd.Series) -> float:
        """计算趋势评分"""
        try:
            close = data['close']
            score = 0
    ...
    def _calculate_momentum_score(self, data: pd.DataFrame, macd_data: Dict) -> float:
        """计算动量评分"""
        try:
            score = 0
            ...
            if 'macd' in macd_data and 'macd_signal' in macd_data:
                ...
                if len(macd) > 0 and len(macd_signal) > 0:
                    ...
                    if not (pd.isna(macd_val) or pd.isna(macd_signal_val)):
    ...
    def _calculate_overbought_oversold_score(self, rsi: pd.Series) -> float:
        """计算超买超卖评分"""
        try:
            score = 0
    ...
    def calculate_strategy_score(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        # 标准化数据列名以兼容策略
        standardized_data = self.standardize_data_columns(data)
        
        for name, strategy in self.strategies.items():
            try:
                # 生成策略信号
                signals = strategy.generate_signals(standardized_data)
...
def test_fixed_screener():
    """测试修复版筛选器"""
    print("🧪 测试修复版增强专业筛选器")
    print("=" * 80)
    
    # 创建筛选器
    screener = FixedEnhancedScreener(
    ...
    if results:
        ...
        filename = f"fixed_screening_test_{timestamp}.json"
