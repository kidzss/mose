...
def save_model(model, optimizer, config, metrics, output_dir, epoch):
    """保存模型和相关信息"""
    try:
        # 创建模型目录
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_dir = os.path.join(output_dir, f'model_v{timestamp}')
        ...
        # 转换metrics中的numpy/torch类型为Python原生类型
        converted_metrics = {}
        for key, value in metrics.items():
            if isinstance(value, dict):
                converted_metrics[key] = {k: float(v) if hasattr(v, 'item') else v
        ...
        with open(summary_path, 'w') as f:
            f.write(f"Model Version: v{timestamp}\n")
            f.write(f"Training Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Epochs Trained: {epoch}\n")
            f.write(f"Final Test MSE: {converted_metrics.get('test_metrics', {}).get('mse', 'N/A')}\n")
            f.write(f"Final Test MAE: {converted_metrics.get('test_metrics', {}).get('mae', 'N/A')}\n")
            f.write(f"Best Validation Loss: {converted_metrics.get('best_val_loss', 'N/A')}\n")

        logger.info(f"模型和相关信息已保存到: {model_dir}")
...
def main():
    ...
    try:
        ...
        # 设置输出目录
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        config['output_dir'] = os.path.join(args.output, f'training_{timestamp}')
        ...
        # 准备数据
        data_processor = DataProcessor()
        symbols = data_processor.get_symbols()
        logger.info(f'获取到 {len(symbols)} 个股票代码')

        # 准备训练数据
        X, y = prepare_data(data_processor, symbols, config)
