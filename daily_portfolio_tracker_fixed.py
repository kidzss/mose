...
class DailyPortfolioTracker:
        ...
        for symbol in all_symbols:
            try:
                ...
                if not hist.empty:
                    ...
                    rsi = self.calculate_rsi(hist_1y['Close']).iloc[-1] if len(hist_1y) > 14 else 50.0
        ...
        for symbol, plan in self.buying_plans.items():
            if symbol in market_data:
                ...
                daily_report['buying_plans_status'][symbol] = {
                    ...
                    'price_condition_met': bool(price_condition),
                    'current_rsi': float(rsi),
                    'target_rsi_range': rsi_range,
                    'rsi_condition_met': bool(rsi_condition),
                    'overall_ready': bool(price_condition and rsi_condition),
                    'days_waiting': int((datetime.now() - datetime.strptime(plan['created_date'], '%Y-%m-%d')).days)
                }
    ...
    def print_summary_and_actions(self, daily_report, execution_ready):
        """打印总结和行动建议"""
        print(f"\n📋 今日总结与行动建议")
        print("=" * 70)
        ...
        # 持仓表现
        if daily_report['holdings_performance']:
        ...
        if execution_ready:
            print(f"\n🚨 紧急行动提醒:")
            for symbol in execution_ready:
                print(f"   🔥 {symbol} 买入条件已满足，建议立即执行分批买入计划！")
        ...
        for symbol, status in daily_report['buying_plans_status'].items():
            ...
            if distance <= 2:  # 距离目标价格2%以内
                print(f"   🔥 {symbol}: 非常接近买入区间，高度关注！")
            elif distance <= 5:  # 距离目标价格5%以内
                print(f"   📍 {symbol}: 接近买入区间，密切关注")
            elif distance <= 10:
                print(f"   👀 {symbol}: 可能即将进入买入区间")
            else:
                print(f"   ⏳ {symbol}: 继续等待回调 (还需回调{distance:.1f}%)")
        
        # 风险提醒
        print(f"\n⚠️ 风险提醒:")
        print("   • 买入计划执行前请再次确认市场环境")
        print("   • 严格按照分批买入计划执行，避免一次性重仓")
        print("   • 设置好止损线，控制风险")
        print("   • 保持足够现金应对突发情况")
    
    def run_daily_analysis(self):
        """运行每日分析"""
        print("📊 每日投资组合分析与买入计划追踪")
        print("=" * 70)
        ...
        # 监控买入计划
        execution_ready = self.monitor_buying_plans(market_data)
        
        # 生成每日报告
        daily_report = self.generate_daily_report(market_data, holdings_analysis, total_value, cash_position)
        
        # 打印总结和行动建议
        self.print_summary_and_actions(daily_report, execution_ready)
